...
export default function AuditEntryDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    Sentry.captureException(error, { tags: { route: 'admin/audit/[id]' } });
  }, [error]);

  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <Card className="max-w-md" role="alert">
        <CardEyebrow>Audit entry error</CardEyebrow>
        <CardTitle>We couldn&apos;t load this audit entry.</CardTitle>
        <CardDescription>
          The data layer returned an unexpected error. Try again, or go back to the audit log. If
          this keeps happening, send the reference below to{' '}
          <a
